...
export function StatisticsClient({ totals, bests, avgs, matchCount, fullStats }: any) {
  ...
  return (
    <div>
      ...
      <RevealGroup className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-6" stagger={0.045}>
        {[
          { icon: Flame, label: "Kills", value: totals.kills },
          { icon: Shield, label: "Blocks", value: totals.blocks },
          { icon: Target, label: "Digs", value: totals.digs },
          { icon: Zap, label: "Aces", value: totals.aces },
          { icon: Award, label: "Assists", value: totals.assists },
          { icon: XIcon, label: "Errors", value: totals.errors },
        ].map((s) => (
      ...
      <RevealGroup className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-6"
        stagger={0.045} delay={0.1}>
        {[
          { icon: Flame, label: "Kills/match", value: avgs.kills },
          { icon: Shield, label: "Blocks/match", value: avgs.blocks },
          { icon: Target, label: "Digs/match", value: avgs.digs },
          { icon: Zap, label: "Aces/match", value: avgs.aces },
          { icon: Award, label: "Assists/match", value: avgs.assists },
          { icon: XIcon, label: "Errors/match", value: avgs.errors },
        ].map((s) => (
          <RevealItem key={s.label}>
            <StatTile icon={s.icon} label={s.label} value={s.value} />
          </RevealItem>
      ...
      <LockedFeature locked={!fullStats} title="Personal bests">
        <Card>
          ...
          {bests && (
            <RevealGroup className="grid grid-cols-2 sm:grid-cols-3 gap-4" stagger={0.05} delay={0.24}>
              {[
                ...
                <RevealItem key={b.label} y={6} className="rounded-2xl p-4 border relative overflow-hidden"
